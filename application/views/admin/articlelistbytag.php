...
你可以在这里管理文章，点击右上角的图标可以关闭该提示。								
        ...
        <span><input name="w" class="editbox_search" id="w" maxlength="80" value="" type="text" /></span><input name="button_search" type="submit" value="搜索文章"/>
        </form>
      </div>					
						<table>
								...
								<tr>
									<td colspan="6">
										<div class="bulk-actions align-left">
											...
											<input type="button" class="button" value="应用" id="article_tag_apply_button">
										</div>
							...
							foreach ($articlelist as $key=>$value):?>
								<tr id="r<?php echo $value->id;?>">
									...
									<td id="d<?php echo $value->id;?>"><?php echo $value->blog_category_name?'<a href="'.site_url('admin/article/listbycategory/'.$value->post_category_id).'" title="查看\''.$value->blog_category_name.'\'下的文章">'.$value->blog_category_name.'</a>':'暂无分类';?></td>
									...
									<td>
										<!-- Icons -->
										 ...
										 <a href="javascript:delete_article_bytag('<?php echo $value->id;?>','<?php echo site_url().'admin/article/deletearticle_bytag';?>','<?php echo $tag_id;?>')" title="删除" id="del<?php echo $value->id;?>"><?php echo img(array("css/admin/images/icons/cross.png",'alt'=>"删除"));?></a> 
									</td>
								</tr>
							<?php endforeach;?>
							<?php endif;?>	
